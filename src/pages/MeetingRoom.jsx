...
import LocalParticipantCard from "../components/meetingRoom/LocalParticipantCard";
...
const MeetingRoom=()=>{
  ...
  const {localVideoRef,localStream,remoteStreams,isMicOn,isVideoOn,toggleMic,toggleVideo,stopLocalStream}=useWebRTC({socket});
  ...
  const joinMeeting=async()=>{
    try{
      ...
      const response=await axios.post(
        `${import.meta.env.VITE_API_URL}/api/v1/meetings/${meetingId}/join`,
        {},
      ...
      setIsHost(meetingData.hostId?.toString()===participant._id?.toString());
      if(!socket.connected)socket.connect();
      socket.emit("joinMeeting",{meetingId,participant});
      setJoined(true);
    }catch(err){
      setErrorMessage(err?.response?.data?.message||"Failed to join meeting");
    }finally{
      setLoading(false);
    }
  };
  ...
  useEffect(()=>{
    const handleParticipants=(users)=>setParticipants(users);
  ...
  const endMeeting=async()=>{
    try{
      await axios.patch(
        `${import.meta.env.VITE_API_URL}/api/v1/meetings/${meetingId}/end`,
        {},
        {withCredentials:true}
      );
  ...
  const remoteParticipants=participants.filter(u=>socket?.id&&u.socketId!==socket.id);

  return(
    <div className="min-h-screen bg-slate-950">
      <MeetingHeader meeting={meeting} isHost={isHost} endMeeting={endMeeting} leaveMeeting={leaveMeeting}/>
      <div className="p-6">
